...
M = 2;L = 2;
lpf1 = LPF(r/(2*M),r,101);
xd1 = decimator(lpf1,x,M);
xi1 = interpolator(lpf1,x,M);
y1 = interpolator(lpf1,xd1,L);
audiowrite('decimated'+M+'.wav',xd1,floor(r/M));
audiowrite('interpolated1.wav',y1,r);
figure()
specgram(xd1,1024,r/M);
figure()
specgram(y1,1024,r);

M = 4;L = 4;
lpf2 = LPF(r/(2*M),r,101);
xd2 = decimator(lpf2,x,4);
xi2 = interpolator(lpf2,x,L);
y2 = interpolator(lpf2,xd2,4);
audiowrite('decimated2.wav',xd2,floor(r/M));
audiowrite('interpolated2.wav',y2,r);
figure()
specgram(xd2,1024,r/M);
figure()
specgram(y2,1024,r);

M = 8;L = 8;
lpf3 = LPF(r/(2*M),r,101);
xd3 = decimator(lpf3,x,8);
xi3 = interpolator(lpf3,x,L);
y3 = interpolator(lpf3,xd3,8);
audiowrite('decimated3.wav',xd3,floor(r/M));
audiowrite('interpolated3.wav',y3,r);
figure()
specgram(xd3,1024,r/M);
figure()
specgram(y3,1024,r);
