...
fxd_y = output(x,r,2,2,25);

function fxd_y = output(x,r,M,L,Q)
    fxd_x = bitshift(x,Q);
    lpf_d = LPF(r/(2*M),r,101,1);
    fxd_lpf_d = fxd_LPF(r/(2*M),r,101,1,Q);
    xd = decimator(lpf_d,x,M);
    fxd_xd = fxd_decimator(fxd_lpf_d,fxd_x,M,Q);

    lpf_i = LPF(r/(2*L),r,101,1);
    fxd_lpf_i = fxd_LPF(r/(2*L),r,101,L,Q);
    ...
    figure("Name","Decimated Output Specgram")
    specgram(f_xd,1024,r/M);
    figure("Name","Interpolated Output Specgram")
    specgram(f_y,1024,r);
    audiowrite("decimated_M=" + M + ".wav",f_xd,floor(r/M));
    audiowrite("interpolated_M=" + M +".wav",f_y,r);
end
...
function Y = convolutionfxd(X,H,Q)
    lenX = length(X);
    lenH = length(H);
    lenY = lenX+lenH-1;
    Y = zeros(lenY,1);
...
function fxd_xd = fxd_decimator(fxd_lpf,fxd_x,M,Q)
    lx = length(fxd_x);
    lh = length(fxd_lpf);
    fxd_xf = convolutionfxd(fxd_x,fxd_lpf,Q);
...
function fxd_y = fxd_interpolator(fxd_lpf,fxd_xd,L,Q)
    ...
    fxd_temp_y = convolutionfxd(fxd_xu,fxd_lpf,Q);
    lh = length(fxd_lpf);
